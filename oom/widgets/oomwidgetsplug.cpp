...
//    $Id: oomwidgetsplug.cpp,v 1.9.2.9 2009/12/01 03:52:40 terminator356 Exp $
//  (C) Copyright 2001-2003 Werner Schweer (ws@seh.de)
//=========================================================

// this file makes some of the OOMidi widgets available
// to QT-Designer
...
class OOMidiPlugin : public QWidgetPlugin
{
public:
...
QStringList OOMidiPlugin::keys() const
{
	QStringList list;
...
QWidget* OOMidiPlugin::create(const QString& key, QWidget* parent,
		const char* name)
{
...
QString OOMidiPlugin::group(const QString& /*key*/) const
{
	return QString("OOMidi");
}
...
QIcon OOMidiPlugin::iconSet(const QString& key) const
{
...
QString OOMidiPlugin::includeFile(const QString& key) const
{
	if (key == QString("PosEdit"))
...
QString OOMidiPlugin::toolTip(const QString& key) const
{
	if (key == QString("PosEdit"))
...
QString OOMidiPlugin::whatsThis(const QString& key) const
{
	if (key == QString("PosEdit"))
		return QString("midi time position editor");
	else if (key == QString("PitchEdit"))
...
bool OOMidiPlugin::isContainer(const QString& /*key*/) const
{
	return false;
}

Q_EXPORT_PLUGIN(OOMidiPlugin)
